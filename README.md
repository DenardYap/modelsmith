...
What you can do with 1-bit data is you can first pack them into "fake" 8-bit data, then perform SIMD instructions (like the one above) on these "fake" 8-bit data, which means, you can process up to 128 binary data at once.

## Why are we implementing True Quantization over Fake Quantization?
...
If you'd like to try it out, go to https://github.com/DenardYap/modelsmith/blob/main/experiments/kernel_test/kernels/test_cases/benchmark.cpp to test out the difference in speed between my custom kernel and PyTorch's kernel
